...
import { DISC_V2_GROUPS, discV2Counts } from "../contracts/assessments";
...
/* 4) DISC MOST/LEAST 双线图 + 反弹区 */
const discV2Ans = { most: DISC_V2_GROUPS.map((g) => g.types.indexOf("D")), least: DISC_V2_GROUPS.map((g) => g.types.indexOf("S")) };
const counts = discV2Counts(discV2Ans);
...
for (const kw of ["反弹区", "物极必反", "校园中的我 · 最像选择（MOST）", "被压在身后的我 · 最不像选择（LEAST）"]) need(discTab, kw, "DISC 双线图+反弹区");
/* MOST 全 D：D=24 进入反弹区（≥80%），应有物极必反提示 */
