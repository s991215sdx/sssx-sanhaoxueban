...
/* 4) DISC 单图竖线折线 + 反弹区 + 分数口径（v43：删除双图，单一 DISC 结果） */
...
for (const kw of ["反弹区", "物极必反", "得分口径", "与上方条形图为同一份分数", "D 型（掌控型）"]) need(discTab, kw, "DISC 单图+反弹区+口径");
if (/校园中的我|被压在身后|MOST|LEAST/.test(discTab)) throw new Error("DISC 双图/自创概念仍在");
/* MOST 全 D、LEAST 全 C：净分 D=24 进入反弹区（≥80%），应有物极必反提示 */
if (!discTab.includes("果敢可能反成专断")) throw new Error("DISC 反弹区提示缺失");
console.log("OK v43 DISC 单图反弹区+口径");
...
console.log("OK v43 SCL-90 评估报告 + 综合集成");

console.log("RENDER_SMOKE_V43_OK");
