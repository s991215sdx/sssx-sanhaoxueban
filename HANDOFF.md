...
## v46（2026-09-14，git 1341736 / 版本 4f15057）
- **条方向反转**：行为特征轴（ReportView DiscBipolarAxis）与亲子对照 FactorBars（DiscParentCompare）的条公式统一改为 `left: t > 0 ? 50 - t/2 : 50`——正值条从中心向左（朝 + 端标签）伸展，负值条从中心向右（朝 - 端标签）伸展；+100% → left:0% 满左半，-100% → left:50% 满右半，0% → 缩为中线。值跟轴走（+N% 左列 / -N% 右列）不变。
